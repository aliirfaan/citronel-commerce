...
class OrderItemIndexController extends OrderController
{
    public function index(Request $request,string $orderGuid, AuditLogService $auditService, CitronelOrderService $orderService)
    {
        ...
        $this->subProcess = $this->errorCatalogueService->getSubProcess($this->mainProcess['key'], 'order_items_index');
        ...
        try{
            $orderItemsResponse = $orderService->getOrderItemsByOrderGuid($orderGuid);
            ...
            if(!$orderItemsResponse['success']){
                ...
                $orderItems = $orderItemsResponse['result']
                    ->paginate($perPage, ['*'], 'page', $pageNumber)
                    ->withQueryString();

                $this->data['success'] = true;
                $this->data['status_code'] = Response::HTTP_OK;
                $this->data['result'] = $orderItems;
            }
